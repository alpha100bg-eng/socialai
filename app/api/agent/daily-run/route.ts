...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── Step 6b (optionnel) : son IA payant via MMAudio ──────────────────
    // Désactivé par défaut : le son est ajouté GRATUITEMENT côté GitHub Actions
    // (musique libre de droits via ffmpeg). Activable avec ENABLE_MMAUDIO=1.
    if (process.env.ENABLE_MMAUDIO === '1') {
      try {
